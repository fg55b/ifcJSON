...
def ifcjson_validator(ifcjson_files_folder):
    with open(schema_file_path, "r") as schema_file:
        ...
        for root, dirs, files in os.walk(ifcjson_files_folder):
                ...
                if file_extension.lower() == '.json':
                    with open(json_file_path, "r") as json_file:
                        ...
                        for ifc_instance in get_all_ifc_types(instance['data']):
                            ...
                            # Particular cases are instances of IfcShapeRepresentation, IfcOwnerHistory and IfcGeometricRepresentationContext,
                            # that does not have globalId in the original express schema but have in ifcJSON. Then, the parameter error must
                            # be ignored in these cases
                            if ifc_type in ['IfcShapeRepresentation', 'IfcOwnerHistory', 'IfcGeometricRepresentationContext']:
                                for validated_parameter in ifc_instance.keys():
                                    if validated_parameter == 'ref':
                                        continue
                                    elif validated_parameter == 'globalId':
                                        continue
